...
export default function HAWards() {
  ...
  async function handleBedStatus(wardId: string, bedId: string, status: string, patientName?: string) {
    setBedLoading(bedId);
    try {
      await api(`/wards/${wardId}/beds/${bedId}`, "PATCH", { status, patientName: patientName || null });
                      ...
                      {beds[ward.id].map(bed => (
                        <div key={bed.id} className={`rounded-lg border p-2 text-center text-xs cursor-pointer transition-all ${STATUS_COLORS[bed.status]}`}
                          onClick={() => {
                            ...
                            const name = next === "occupied" ? prompt("Patient name (optional):") || undefined : undefined;
                            handleBedStatus(ward.id, bed.id, next, name);
                          }}>
                          <p className="font-bold">{bed.bed_number}</p>
                          <p className="capitalize mt-0.5 opacity-80">{bed.status}</p>
