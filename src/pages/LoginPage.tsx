...
export default function LoginPage({
  ...
  // ── Native Google Sign-In (app only) ────────────────────────────────────────
  ...
  const googleSection = GOOGLE_CLIENT_ID && screen === "patient-form" && !IS_NATIVE ? (
  ...
  const nativeGoogleSection = IS_NATIVE && screen === "patient-form" ? (
    <div className="space-y-3 mb-4">
      {googleLoading
        ? <div className="flex justify-center items-center gap-2 text-sm text-gray-500 py-2.5">
            <Loader2 className="w-4 h-4 animate-spin text-teal-500" /> Signing in with Google…
        ...
        : <button
            ...
            className="w-full flex items-center justify-center gap-3 border border-gray-300 rounded-full py-2.5 px-4 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
          >
